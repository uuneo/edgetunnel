...
let ChatID;
let pushhosts = [];
let pushhostsURL = atob(
  "aHR0cHM6Ly9yYXcuZ2l0aHVidXNlcmNvbnRlbnQuY29tL2NtbGl1L2NtbGl1L21haW4vUHJveHlIT1NU"
);
let RpushIP = "false";
...
export default {
  async fetch(request, env, ctx) {
    try {
      ...
      const fakeHostName = `${fakeUserIDMD5.slice(6, 9)}.${fakeUserIDMD5.slice(
        13,
        19
      )}`;

      pushIP = env.PROXYIP || env.puship || pushIP;
      pushIPs = await 整理(pushIP);
      pushIP = pushIPs[Math.floor(Math.random() * pushIPs.length)];
      ...
      enableHttp = env.HTTP
        ? true
        : socks5Address.toLowerCase().includes("http://");
      ...
      if (socks5Address) {
        try {
          ...
          RpushIP = env.RPROXYIP || "false";
          enableSocks = true;
        } catch (err) {
          let e = err;
          console.log(e.toString());
          RpushIP = env.RPROXYIP || !pushIP ? "true" : "false";
          enableSocks = false;
        }
      } else {
        RpushIP = env.RPROXYIP || !pushIP ? "true" : "false";
      ...
      if (!upgradeHeader || upgradeHeader !== "websocket") {
        ...
        if (url.searchParams.has("puship")) {
          path = `/puship=${url.searchParams.get("puship")}`;
          RpushIP = "false";
        ...
        if (路径 == "/") {
          ...
          url.pathname == `/${动态推特名字}/edit` ||
          路径 == `/${userID}/edit`
        ) {
          return await KV(request, env);
        } else if (
          url.pathname == `/${动态推特名字}/bestip` ||
          路径 == `/${userID}/bestip`
        ) {
          return await bestIP(request, env);
        } else if (url.pathname == `/${动态推特名字}` || 路径 == `/${userID}`) {
          ...
          if (
            (env.CF_EMAIL && env.CF_APIKEY) ||
            (env.CF_ID && env.CF_APITOKEN)
          ) {
        ...
        if (new RegExp("/socks5=", "i").test(url.pathname))
          ...
          if (socks5Address.includes("@")) {
            ...
            let userPassword = socks5Address
            ...
            socks5Address = `${userPassword}@${socks5Address.substring(
              lastAtIndex + 1
            )}`;
          }
        ...
        if (url.searchParams.has("puship")) {
          pushIP = url.searchParams.get("puship");
          enableSocks = false;
        } else if (new RegExp("/puship=", "i").test(url.pathname)) {
          pushIP = url.pathname.toLowerCase().split("/puship=")[1];
          enableSocks = false;
        } else if (new RegExp("/puship.", "i").test(url.pathname)) {
          pushIP = `puship.${
            url.pathname.toLowerCase().split("/puship.")[1]
          }`;
          enableSocks = false;
        } else if (new RegExp("/pyip=", "i").test(url.pathname)) {
          pushIP = url.pathname.toLowerCase().split("/pyip=")[1];
          enableSocks = false;
        }
...
async function handleTCPOutBound(
  ...
  async function connectAndWrite(address, port, socks = false, http = false) {
    ...
    const tcpSocket = socks
      ? http
        ? await httpConnect(address, port, log)
  ...
  async function retry() {
    if (enableSocks) {
      ...
      if (!pushIP || pushIP == "") {
        pushIP = atob("UFJPWFlJUC50cDEuMDkwMjI3Lnh5eg==");
      } else if (pushIP.includes("]:")) {
        portRemote = pushIP.split("]:")[1] || portRemote;
        pushIP = pushIP.split("]:")[0] + "]" || pushIP;
      } else if (pushIP.split(":").length === 2) {
        portRemote = pushIP.split(":")[1] || portRemote;
        pushIP = pushIP.split(":")[0] || pushIP;
      }
      if (pushIP.includes(".tp"))
        portRemote = pushIP.split(".tp")[1].split(".")[0] || portRemote;
      tcpSocket = await connectAndWrite(
        pushIP.toLowerCase() || addressRemote,
        portRemote
      );
    }
  ...
  // 首次尝试连接远程服务器
  let tcpSocket = await connectAndWrite(
    addressRemote,
    portRemote,
    useSocks,
    enableHttp
        ...
        async write(chunk) {
          if (webSocket.readyState === WS_READY_STATE_OPEN) {
            if (维列斯Header) {
              ...
              webSocket.send(
                await new Blob([维列斯Header, chunk]).arrayBuffer()
              );
    ...
    while (true) {
      ...
      if (respText.includes("\r\n\r\n")) {
        ...
        if (
          ...
          headers.startsWith("HTTP/1.0 200")
        ) {
          connected = true;
          ...
          if (headersEndPos < responseBuffer.length) {
            ...
            dataStream
              .pipeTo(writable)
              .catch((err) => console.error("处理剩余数据错误:", err));
...
function 配置信息(UUID, 域名地址) {
  ...
  const 威图瑞 =
    `${协议类型}://${用户ID}@${地址}:${端口}\u003f\u0065\u006e\u0063\u0072\u0079` +
    "p" +
    `${
      atob("dGlvbj0=") + 加密方式
    }\u0026\u0073\u0065\u0063\u0075\u0072\u0069\u0074\u0079\u003d${
      传输层安全[0]
    }&sni=${SNI}&fp=${指纹}&type=${传输层协议}&host=${伪装域名}&path=${
      encodeURIComponent(路径) + allowInsecure
...
async function 生成配置信息(
  ...
  const uuid = _url.pathname == `/${动态推特名字}` ? 动态推特名字 : userID;
  const userAgent = UA.toLowerCase();
  ...
  if (hostName.includes(".workers.dev")) {
    if (pushhostsURL && (!pushhosts || pushhosts.length == 0)) {
      try {
        const response = await fetch(pushhostsURL);
        ...
        pushhosts = pushhosts.concat(nonEmptyLines);
      } catch (error) {
        //console.error('获取地址时出错:', error);
      }
    }
    if (pushhosts.length != 0)
      pushhost =
        pushhosts[Math.floor(Math.random() * pushhosts.length)] + "/";
  }
  ...
  if (
    ...
    if (go2Socks5s.length > 0 && enableSocks) {
      socks5List = `${
        (enableHttp ? "HTTP" : "Socks5") +
        decodeURIComponent(
          "%EF%BC%88%E7%99%BD%E5%90%8D%E5%8D%95%EF%BC%89%3A%20"
        )
      }`;
      if (
        go2Socks5s.includes(atob("YWxsIGlu")) ||
    ...
    if (sub) {
      ...
      else if (pushIP && pushIP != "")
        订阅器 += `CFCDN（访问方式）: ProxyIP<br>&nbsp;&nbsp;${pushIPs.join(
          "<br>&nbsp;&nbsp;"
        )}<br>`;
      else if (RpushIP == "true")
        订阅器 += `CFCDN（访问方式）: 自动获取ProxyIP<br>`;
      else
        订阅器 += `CFCDN（访问方式）: 内置兜底, 您也可以设置 pushIP/PROXYIP 。<br>`;
      ...
      let 判断是否绑定KV空间 = "";
      if (env.KV)
        判断是否绑定KV空间 = ` [<a href='${_url.pathname}/edit'>编辑优选列表</a>]  [<a href='${_url.pathname}/bestip'>在线优选IP</a>]`;
      订阅器 += `<br>您的订阅内容由 内置 addresses/ADD* 参数变量提供${判断是否绑定KV空间}<br>`;
      if (addresses.length > 0)
        订阅器 += `ADD（TLS优选域名&IP）: <br>&nbsp;&nbsp;${addresses.join(
          "<br>&nbsp;&nbsp;"
        )}<br>`;
      if (addressesnotls.length > 0)
        订阅器 += `ADDNOTLS（noTLS优选域名&IP）: <br>&nbsp;&nbsp;${addressesnotls.join(
          "<br>&nbsp;&nbsp;"
        )}<br>`;
      if (addressesapi.length > 0)
        订阅器 += `ADDAPI（TLS优选域名&IP 的 API）: <br>&nbsp;&nbsp;${addressesapi.join(
          "<br>&nbsp;&nbsp;"
        )}<br>`;
      if (addressesnotlsapi.length > 0)
        订阅器 += `ADDNOTLSAPI（noTLS优选域名&IP 的 API）: <br>&nbsp;&nbsp;${addressesnotlsapi.join(
          "<br>&nbsp;&nbsp;"
        )}<br>`;
      if (addressescsv.length > 0)
        订阅器 += `ADDCSV（IPTest测速csv文件 限速 ${DLS} ）: <br>&nbsp;&nbsp;${addressescsv.join(
          "<br>&nbsp;&nbsp;"
        )}<br>`;
    }

    if (动态推特名字 && _url.pathname !== `/${动态推特名字}`) 订阅器 = "";
    else
      订阅器 += `<br>SUBAPI（订阅转换后端）: <a href='${subProtocol}://${subConverter}/version' target="_blank" rel="noopener noreferrer">${subProtocol}://${subConverter}</a><br>SUBCONFIG（订阅转换配置文件）: <a href='${subConfig}' target="_blank" rel="noopener noreferrer">${subConfig}</a>`;
    const 动态推特名字信息 =
      uuid != userID
        ? `TOKEN: ${uuid}<br>UUIDNow: ${userID}<br>UUIDLow: ${userIDLow}<br>${userIDTime}TIME（动态推特名字有效时间）: ${有效时间} 天<br>UPTIME（动态推特名字更新时间）: ${更新时间} 时（北京时间）<br><br>`
        : `${userIDTime}`;
    const 节点配置页 = `
            ################################################################<br>
            ...
            ---------------------------------------------------------------<br>
            自适应订阅地址:<br>
            <a href="javascript:void(0)" onclick="copyToClipboard('https://${pushhost}${hostName}/${uuid}?sub','qrcode_0')" style="color:blue;text-decoration:underline;cursor:pointer;">https://${pushhost}${hostName}/${uuid}</a><br>
            <div id="qrcode_0" style="margin: 10px 10px 10px 10px;"></div>
            Base64订阅地址:<br>
            <a href="javascript:void(0)" onclick="copyToClipboard('https://${pushhost}${hostName}/${uuid}?b64','qrcode_1')" style="color:blue;text-decoration:underline;cursor:pointer;">https://${pushhost}${hostName}/${uuid}?b64</a><br>
            <div id="qrcode_1" style="margin: 10px 10px 10px 10px;"></div>
            clash订阅地址:<br>
            <a href="javascript:void(0)" onclick="copyToClipboard('https://${pushhost}${hostName}/${uuid}?clash','qrcode_2')" style="color:blue;text-decoration:underline;cursor:pointer;">https://${pushhost}${hostName}/${uuid}?clash</a><br>
            <div id="qrcode_2" style="margin: 10px 10px 10px 10px;"></div>
            singbox订阅地址:<br>
            <a href="javascript:void(0)" onclick="copyToClipboard('https://${pushhost}${hostName}/${uuid}?sb','qrcode_3')" style="color:blue;text-decoration:underline;cursor:pointer;">https://${pushhost}${hostName}/${uuid}?sb</a><br>
            <div id="qrcode_3" style="margin: 10px 10px 10px 10px;"></div>
            loon订阅地址:<br>
            <a href="javascript:void(0)" onclick="copyToClipboard('https://${pushhost}${hostName}/${uuid}?loon','qrcode_5')" style="color:blue;text-decoration:underline;cursor:pointer;">https://${pushhost}${hostName}/${uuid}?loon</a><br>
            ...
            <strong><a href="javascript:void(0);" id="noticeToggle" onclick="toggleNotice()">实用订阅技巧∨</a></strong><br>
                <div id="noticeContent" class="notice-content" style="display: none;">
                    ...
                    <br>
                    <strong>3.</strong> 快速切换 <a href='${atob(
                      "aHR0cHM6Ly9naXRodWIuY29tL2NtbGl1L1dvcmtlclZsZXNzMnN1Yg=="
                    )}'>优选订阅生成器</a> 至：sub.google.com，您可将"?sub=sub.google.com"参数添加到链接末尾，例如：<br>
                    &nbsp;&nbsp;https://${pushhost}${hostName}/${uuid}<strong>?sub=sub.google.com</strong><br>
                    <br>
                    <strong>4.</strong> 快速更换 PROXYIP 至：puship.cmliussss.net:443，您可将"?puship=puship.cmliussss.net:443"参数添加到链接末尾，例如：<br>
                    &nbsp;&nbsp; https://${pushhost}${hostName}/${uuid}<strong>?puship=puship.cmliussss.net:443</strong><br>
                    <br>
                    <strong>5.</strong> 快速更换 SOCKS5 至：user:password@127.0.0.1:1080，您可将"?socks5=user:password@127.0.0.1:1080"参数添加到链接末尾，例如：<br>
                    &nbsp;&nbsp;https://${pushhost}${hostName}/${uuid}<strong>?socks5=user:password@127.0.0.1:1080</strong><br>
                    <br>
                    <strong>6.</strong> 如需指定多个参数则需要使用'&'做间隔，例如：<br>
                    &nbsp;&nbsp;https://${pushhost}${hostName}/${uuid}?sub=sub.google.com<strong>&</strong>puship=puship.cmliussss.net<br>
                </div>
    ...
    let url = `${subProtocol}://${sub}/sub?host=${fakeHostName}&uuid=${
      fakeUserID + atob("JmVkZ2V0dW5uZWw9Y21saXUmcHJveHlpcD0=") + RpushIP
    ...
    if (
      ...
      url = `${subProtocol}://${subConverter}/sub?target=loon&url=${encodeURIComponent(
        url
      )}&insert=false&config=${encodeURIComponent(
        subConfig
      )}&emoji=${subEmoji}&list=false&tfo=false&scv=${SCV}&fdn=false&sort=false&new_name=true`;
        ...
        const response = await fetch(url, {
          headers: {
            "User-Agent":
              "v2rayN" +
              atob(
                "L2VkZ2V0dW5uZWwgKGh0dHBzOi8vZ2l0aHViLmNvbS9jbWxpdS9lZGdldHVubmVsKQ=="
              ),
          },
        });
        ...
        if (lines[0].split(",").length > 3) {
          ...
          for (let i = 1; i < lines.length; i++) {
            ...
            if (columns) {
              ...
              if (api[index].includes("puship=true"))
                pushIPPool.push(`${columns}:${测速端口}`);
            }
          }
        } else {
          // 验证当前apiUrl是否带有'puship=true'
          if (api[index].includes("puship=true")) {
            // 如果URL带有'puship=true'，则将内容添加到pushIPPool
            pushIPPool = pushIPPool.concat(
              (await 整理(content))
                .map((item) => {
    ...
    try {
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        if (
          ...
          if (
            csvUrl.includes("puship=true") &&
            columns[tlsIndex].toUpperCase() == "true" &&
            !httpsPorts.includes(port)
          ) {
            // 如果URL带有'puship=true'，则将内容添加到pushIPPool
            pushIPPool.push(`${ipAddress}:${port}`);
          }
        }
      }
    } catch (error) {
...
function 生成本地订阅(
  ...
  if (noTLS == "true") {
    ...
    notlsresponseBody = uniqueAddressesnotls
      .map((address) => {
        ...
        const ctx = `${协议类型}://${UUID}@${address}:${
          port +
          atob("P2VuY3J5cHRpb249bm9uZSZzZWN1cml0eT0mdHlwZT13cyZob3N0PQ==") +
          伪装域名
        }&path=${encodeURIComponent(最终路径)}#${encodeURIComponent(
          addressid + 节点备注
        )}`;
  ...
  const responseBody = uniqueAddresses
    .map((address) => {
      ...
      let 最终路径 = path;
      let 节点备注 = "";
      const matchingProxyIP = pushIPPool.find((pushIP) =>
        pushIP.includes(address)
      );
      if (matchingProxyIP) 最终路径 = `/puship=${matchingProxyIP}`;
      /*
        if (pushhosts.length > 0 && (伪装域名.includes('.workers.dev'))) {
            最终路径 = `/${伪装域名}${最终路径}`;
            伪装域名 = pushhosts[Math.floor(Math.random() * pushhosts.length)];
      ...
      const ctx = `${协议类型}://${UUID}@${address}:${
        ...
        伪装域名
      }&fp=random&type=ws&host=${伪装域名}&path=${
        encodeURIComponent(最终路径) + allowInsecure
      }&fragment=${encodeURIComponent(
        "1,40-60,30-50,tlshello"
    ...
    const html = `
            ...
            <body>
                ...
                <div id="noticeContent" class="notice-content">
                    ${decodeURIComponent(
                      atob(
                        ""
                      )
                    )}
                </div>
...
async function resolveToIPv6(target) {
  ...
  function convertToNAT64IPv6(ipv4Address) {
    ...
    // 构造NAT64
    return (
      DNS64Server.split("/96")[0] + hex[0] + hex[1] + ":" + hex[2] + hex[3]
    );
  }
  ...
  try {
    ...
    const nat64 = DNS64Server.endsWith("/96")
      ? convertToNAT64IPv6(ipv4)
      : await queryNAT64(ipv4 + atob("LmlwLjA5MDIyNy54eXo="));
...
async function bestIP(request, env, txt = "ADD.txt") {
  ...
  async function getNipDomain() {
    try {
      const response = await fetch(
        atob(
          "aHR0cHM6Ly9jbG91ZGZsYXJlLWRucy5jb20vZG5zLXF1ZXJ5P25hbWU9bmlwLjA5MDIyNy54eXomdHlwZT1UWFQ="
        ),
        {
          headers: {
  ...
  async function GetCFIPs(ipSource = "official", targetPort = "443") {
    try {
      ...
      if (ipSource === "as13335") {
        ...
        console.log(
          `反代IP列表解析完成，端口${targetPort}匹配到${validIps.length}个有效IP`
        );
...
131.0.72.0/22`;
      ...
      while (ips.size < targetCount) {
        ...
        for (const cidr of cidrs) {
          ...
          console.log(
            `CIDR ${cidr} 第${round}轮生成${cidrIPs.length}个IP，总计${ips.size}个`
          );
        }
  ...
  const countryDisplayClass = isChina ? "" : "push-warning";
    ...
    <div class="test-controls">
        <div class="port-selector">
            ...
            <select id="ip-source-select">
                ...
                <option value="puship">反代IP列表</option>
            </select>
...
async function getUsage(accountId, email, apikey, apitoken, all = 100000) {
  ...
  async function getAccountId(email, apikey) {
    ...
    if (!response.ok) {
      ...
      console.error(
        `获取账户信息失败: ${response.status} ${response.statusText}`,
        errorText
      );
      throw new Error(
        `Cloudflare API 请求失败: ${response.status} ${response.statusText} - ${errorText}`
      );
    }
  ...
  try {
    ...
    if (!response.ok) {
      const errorText = await response.text();
      console.error(
        `GraphQL查询失败: ${response.status} ${response.statusText}`,
        errorText
      );
    ...
    console.log(
      `统计结果 - Pages: ${pages}, Workers: ${workers}, 总计: ${total}`
    );
